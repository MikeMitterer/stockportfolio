...
describe('Währung in Tageswerten und Sicherungen', () => {
  it('mischt beim Laden keine verschiedenen Währungen', async () => {
    ...
    await store.record('depot', 125, 'EUR', new Date('2026-09-10T12:00:00Z'))
    await store.load('depot', 'EUR')
    expect(store.snapshots).toContainEqual({ date: '2026-09-10', total: 125, currency: 'EUR' })
    await store.load('depot', 'USD')
    expect(store.snapshots).toEqual([{ date: '2026-09-10', total: 100, currency: 'USD' }])
    expect(await store.exportAll('depot')).toHaveLength(3)
  ...
  it('erhält Depotwährung und Geldschwellen und verwirft nicht zuordenbare Tageswerte', () => {
    ...
    expect(result.backup.valueHistory).toEqual([{ date: '2026-09-09', total: 180, currency: 'EUR' }, ...backup.valueHistory])
