...
import { MOCK_PORTFOLIO, MOCK_QUOTES, MOCK_SETTINGS } from '@/mock/portfolio'

const { t } = useI18n()

/**
 * Preview-Modus: Domain-Aggregator direkt mit Mock-Daten aufrufen.
 * Nach T-05 wird das durch reaktive Pinia-Stores ersetzt.
 */
const result = computed(() =>
  computeRebalancing(MOCK_PORTFOLIO, MOCK_QUOTES, MOCK_SETTINGS),
)
...
<template>
  <div class="max-w-[1400px] mx-auto px-6 py-6 flex flex-col gap-6">
    ...
    <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      />
      <KpiCard :label="t('kpi.warnings')" :value="t('kpi.warningsNone')" tone="positive" />
    </section>

    <!-- Gruppen-Balken (ein-/ausklappbar) -->
    <section class="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 overflow-hidden">
      <button
        type="button"
      ...
      >
        ...
        <h2 class="text-xs uppercase tracking-wide text-neutral-500 dark:text-neutral-400 font-medium">
          Assetklassen
        </h2>
        <span
          v-if="groupsCollapsed"
          class="ml-auto text-xs text-neutral-500 tabular-nums"
        >
    ...
    <section class="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 overflow-hidden">
      <div class="px-5 py-4 flex items-baseline justify-between">
        ...
        <div class="text-xs text-neutral-500 tabular-nums">
          Bänder: −{{ percent(MOCK_SETTINGS.bands.lowerPercent) }} / +{{ percent(MOCK_SETTINGS.bands.upperPercent) }}
        </div>
      </div>
      <NDivider class="!my-0" />
