...
- [x] Rote Tests für Quote, Refresh, Katalog und Persistenz schreiben:
  ```ts
  ...
  ```
- [x] `npm test -- --run tests/api/details.spec.ts` ausführen und Verlust der
  neuen Werte beziehungsweise fehlenden Clientweg belegen.
- [x] `/fields` und Details an derselben API-Grenze prüfen. Unbekannte additive
  Felder ignorieren. Ungültige Zusatzwerte dürfen gültige Core-Kurse nicht
  unbrauchbar machen; keine Ersatzwährung verwenden.
- [x] Mapper erhalten sämtliche gültigen Wertmetadaten. Speichern, Einzel- und
...
- [x] Rote Projektionstests: 0, false, null, zwei gleich beschriftete Namespaces,
  ...
  ```
- [x] Nur definierte, anwendbare Felder darstellen; vollständige Feldnamen
...
- [x] Sitzungsspeicher, Fehlerfall, paralleles Öffnen und Adresswechsel prüfen.
...
- [x] Rote Komponententests für die tatsächliche Tabellen-/Drilldown-Verbindung.
...
- [x] Dynamisches Feld zur Hauptzeile hinzufügen und entfernen; nur tatsächlich
  dargestellte Felder ausschließen. Kern-TER/-Volatilität nicht doppelt zeigen.
- [x] Alle Werte ausschließlich als Text darstellen. Fehlender Katalog und noch
